...
import {
  ...
  Music2, AlertCircle, Sparkles, Users,
} from "lucide-react";
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2 text-sm text-muted-foreground bg-[#F4F1EA] rounded-lg px-3 py-2.5">
        ...
        <span>Your bar analysis will be shared with the community so others can skip setup for <span className="text-foreground font-medium">{pieceTitle}</span>.</span>
      </div>
...
function PageRangeStep({
  sheetMusicId, initialPageCount, onStarted, onBack,
}: {
  sheetMusicId: number;
  ...
  const startProcess = useMutation({
    mutationFn: async ({ firstPage, lastPage }: { firstPage: number; lastPage: number }) => {
      await apiRequest("POST", `/api/sheet-music/${sheetMusicId}/process`, { firstPage, lastPage });
    },
    onSuccess: () => onStarted(),
    onError: () => {
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-muted-foreground">
        Choose which pages contain <span className="text-foreground font-medium">this piece</span> only — skip title pages, preludes, or other scores.
      </p>
      ...
      {!loading && pdfPageCount > 0 && (
        <div className="space-y-4 rounded-xl border bg-muted/20 p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="contrib-page-from">First page</Label>
              <Input
          ...
          <p className="text-xs text-muted-foreground">
            Bars will be detected on pages {Math.min(fromPage, toPage)}–{Math.max(fromPage, toPage)} ({Math.abs(toPage - fromPage) + 1} page{Math.abs(toPage - fromPage) === 0 ? "" : "s"}).
          </p>
        </div>
      )}
      ...
      <div className="flex gap-3">
        ...
        >
          {startProcess.isPending ? (
            ...
            <>Analyse pages <ChevronRight className="w-4 h-4 ml-1" /></>
          )}
        </Button>
      </div>
    </div>
    ...
    <div className="flex flex-col items-center gap-6 py-6">
      ...
      <div className="text-center space-y-1 w-full">
        {failed ? (
          ...
          <>
            <p className="font-semibold">Processing page {page} of {total}</p>
...
function ShareStep({
  pieceTitle, totalMeasures, sheetMusicId, movementId, userId, onShared, onSkip,
}: {
  pieceTitle: string;
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-xl bg-[#F4F1EA] border border-[#D6D1C7] p-4 flex items-start gap-3">
        ...
        <div>
          <p className="font-semibold text-sm">Your analysis is ready</p>
          <p className="text-xs text-muted-foreground mt-0.5">
            {totalMeasures} bars detected for <span className="text-foreground">{pieceTitle}</span>.
            Contributing it lets every Réperto user skip the setup step for this piece.
          </p>
        </div>
      </div>

      <div className="space-y-2">
        <Label htmlFor="contrib-desc">Edition notes <span className="text-muted-foreground font-normal">(optional)</span></Label>
        <Textarea
          id="contrib-desc"
...
function DoneStep({ pieceTitle, onClose }: { pieceTitle: string; onClose: () => void }) {
    ...
    <div className="flex flex-col items-center gap-5 py-8 text-center">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Your bar analysis for <span className="text-foreground">{pieceTitle}</span> is now
...
export function ContributeScoreWizard({
  open, onOpenChange, pieceId, movementId = null, pieceTitle, userId, onContributed,
}: Props) {
