    ...
    return (
        <div className="space-y-5">
            ...
            {communityScore && onUseCommunityScore && (
                <div className="rounded-xl border border-[#DCCAA6] bg-[#FAF8F3] p-4 flex items-center gap-3">
                    ...
                    <Button
                        size="sm"
                        className="shrink-0"
                        onClick={() => onUseCommunityScore(communityScore)}
                    >
                        Use this →
                    </Button>
                </div>
            ...
            <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/40 rounded-lg px-3 py-2.5">
                ...
                <span>
                    We'll automatically detect every bar so you can track your
    ...
    return (
        <div className="space-y-5">
            <p className="text-sm text-muted-foreground">
                Your PDF may include a title page, preface, or other scores.
            ...
            {!loading && pdfPageCount > 0 && (
                <div className="space-y-4 rounded-xl border bg-muted/20 p-4">
                    ...
                    <p className="text-xs text-muted-foreground">
                        We’ll only detect bars on pages{" "}
    ...
    if (measures.length === 0) {
        return (
            <div className="flex flex-col items-center gap-4 py-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                    No bars were detected. The PDF may be a scan or use
                    non-standard notation. You can still create a plan and enter
                    measures manually.
                </p>
                <div className="flex gap-3">
...
function ConfirmStep({
    ...
    const measuresPerDay =
        totalMeasures > 0 ? Math.ceil(totalMeasures / days) : null;
        ...
        <div className="space-y-5">
            <div className="rounded-xl border bg-muted/20 divide-y divide-border overflow-hidden">
                {[
                    ...
                    {
                        label: "Target date",
                        value: new Date(targetDate).toLocaleDateString(
                            "en-US",
                        ...
                        ? [
                              {
                                  ...
                                  value: totalMeasures.toString(),
                              },
                              {
                                  label: "Measures/day",
                                  value: measuresPerDay?.toString() ?? "—",
                              },
                          ]
                        : []),
                ].map(({ label, value }) => (
            ...
            <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/40 rounded-lg px-3 py-2.5">
                ...
                <span>
                    We'll create a lesson for each day, assigning measures in
                    order so you always know exactly what to practice.
                </span>
            </div>
...
export function LearningPlanWizard({
    ...
    const [pdfSourcePageCount, setPdfSourcePageCount] = useState<number | null>(
        null,
    );
    ...
    const { data: communityScore, refetch: refetchCommunityScore } =
        useQuery<CommunityScoreInfo | null>({
    ...
    const confirmPlan = useMutation({
        mutationFn: async (): Promise<number> => {
            if (sheetMusicId) {
                await apiRequest(
                    "POST",
                    `/api/sheet-music/${sheetMusicId}/confirm`,
                    {},
                );
            }

            const existingRes = await fetch(
                `/api/learning-plans/entry/${repertoireEntryId}`,
                {
                    headers: getAuthHeaders(),
                },
            );
            const existing = existingRes.ok
                ? ((await existingRes.json()) as { id: number } | null)
                : null;
            ...
            if (existing?.id) {
                await apiRequest(
                    "PATCH",
                    `/api/learning-plans/${existing.id}`,
                    payload,
                );
                planId = existing.id;
            } else {
            ...
            await apiRequest(
                "POST",
                `/api/learning-plans/${planId}/generate-lessons`,
                {},
            );
            return planId;
        },
        onSuccess: (planId) => {
            ...
            toast({
                title: "Plan created!",
                description: "Your daily lessons are ready.",
            });
            onOpenChange(false);
            onSuccess?.(planId);
        },
                ...
                {step === "upload" && (
                    <UploadStep
                        ...
                        onUseCommunityScore={(score) => {
                            apiRequest(
                                "POST",
                                `/api/community-scores/${score.id}/use`,
                            ).catch(() => {});
